...
(function () {
	...
	async function http(url, options = {}) {
		...
		if (!res.ok) {
			...
			try { message = JSON.parse(maybeJson).message || maybeJson; } catch (_) { /* ignore */ }
	...
	async function deleteStudent(id) {
		return http(`${API.student}/${encodeURIComponent(id)}`, { method: "DELETE" });
	}
	...
	async function deleteCourse(id) {
		return http(`${API.course}/${encodeURIComponent(id)}`, { method: "DELETE" });
	}

	async function enrollStudent(courseId, studentId) {
		return http(`${API.course}/${encodeURIComponent(courseId)}/enroll-student`, {
			method: "POST",
	...
	async function gradeStudent(courseId, studentId, grade) {
		const url = `${API.grade}/${encodeURIComponent(courseId)}?studentId=${encodeURIComponent(studentId)}`;
		return http(url, {
			method: "POST",
	...
	function populateStudentSelects(students) {
		...
		for (const sel of selects) {
			const el = qs(sel);
			const prev = el.value;
			el.innerHTML = students.map(s => `<option value="${s.id}">${s.nama ?? ("Student " + s.id)}</option>`).join("");
	...
	function populateCourseSelects(courses) {
		...
		for (const sel of selects) {
			const el = qs(sel);
			const prev = el.value;
			el.innerHTML = courses.map(c => `<option value="${c.id}">${c.title ?? ("Course " + c.id)} (${c.instructor ?? ""})</option>`).join("");
			if (prev) el.value = prev;
		}
	}
	...
	function renderEnrolled(enrollments, nama) {
		...
		populateGradeStudentSelects(enrollments.map(e => ({ id: e.studentId ?? e.student_id , nama: e.Student ? e.Student.nama : nama })));
	...
	function wireEnrollmentAndGrades() {
		const enrollForm = qs("#enroll-form");
		...
		loadEnrolledBtn.addEventListener("click", async () => {
			const courseId = qs("#enrolled-course").value;
			const studentId = qs("#enroll-student").value;
			try {
				const data = await getEnrolled(courseId);
				const student = await getStudent(studentId);
				const studentName = student ? (student.nama || `Student ${student.id}`) : "";
				renderEnrolled(data || [], studentName);
			} catch (err) {
		...
		const table = qs("#grade-table");
		gradeForm.addEventListener("submit", async (e) => {
			e.preventDefault();
			const courseId = qs("#grade-course").value;
			const studentId = qs("#grade-student").value;
		...
		table.addEventListener("click", async (e) => {
			const loadGradeBtn = qs("#load-grade");
		});
		qs("#grade-course").addEventListener("change", async (e) => {
  const courseId = e.target.value;
  if (!courseId) return;
  try {
    await listGrades(courseId);
  } catch (err) {
    toast(err.message || "Failed to load grades", "error");
  }
});
